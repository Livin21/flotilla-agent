...
/*
 * The set of form actions this page owns.
 */
const FLOTILLA_ACTIONS = ['pair', 'reset', 'save', 'repair_entities', 'repair_cron', 'test'];

/*
 * THE POST HANDLER ONLY RUNS WHEN THIS FILE IS THE DIRECTLY-REQUESTED SCRIPT
 * (the page's forms post to /plugins/flotilla-agent/include/settings.php, the same pattern
 * Community Applications uses for its /plugins/.../scripts/*.php endpoints; nginx routes any
 * .php to php-fpm behind Unraid's session auth). Including this file is now ALWAYS
 * side-effect-free.
 *
 * Why it must never run at include time: Unraid also includes this file while RENDERING the
 * settings page, and it renders the page for POST requests too. Browsers re-submit the last
 * POST on reload -- in the field, a tab whose last action was the 1.0 "Pair" click re-POSTed
 * `action=pair` (with no csrf_token; the 1.0 form predates the CSRF field) on EVERY reload,
 * for days. Two prior guards both failed against that:
 *   - blanket `REQUEST_METHOD === 'POST'`: hijacked every POST render;
 *   - method + action-allowlist: still hijacked it, because `pair` IS one of our actions.
 * Each time, the CSRF check failed and exit()'d MID-RENDER. By then Unraid has already emitted
 * the page head, so `header('Location: ...')` was a silent no-op and the response simply
 * truncated: a BLANK settings page with nothing in /var/log/phplog, nginx's log, or the browser
 * console -- an exit() is neither a PHP error nor a catchable Throwable, so even Unraid's
 * evalContent.php try/catch cannot see it. Every CLI check passes (php_sapi_name() is 'cli').
 * Only the real page in a real browser fails.
 *
 * Handling the POST as the direct request script instead means headers are NOT yet sent, so the
...
if ($flotillaIsDirect && ($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
  // Someone navigated to the endpoint itself -- send them to the settings page.
