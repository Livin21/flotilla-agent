...
# I2: the bearer secret S goes to curl through a config file on a process-substituted fd
# (/dev/fd/N), NOT as `-H "Authorization: Bearer $SECRET"`. Same threat model as
# FLOTILLA_SEAL_KEY six lines up, which this call site used to contradict:
# /proc/<pid>/cmdline is world-readable on Linux (Unraid sets no hidepid), so any process in
# the host PID namespace -- most realistically a container run with --pid=host, which does not
# even need to be root -- could poll it and recover S. S alone can't decrypt anything (K never
# leaves the 0600 cfg), but it is enough to POST a "going-down" heartbeat that permanently
# silences the relay's dead-man switch while the server is genuinely down, to DELETE the
# pairing outright, or to spam the phone. Process substitution keeps it off argv without ever
# writing it to disk (no mktemp to leak on a crash, nothing to clean up).
#
# The body moves to stdin (`-d @-`) for the same reason -- it carried pairingID and the sealed
# ciphertext on argv -- matching what the heartbeat send has always done.
#
# Quoting is safe by construction: SECRET is base64url (A-Za-z0-9-_ only), generated by
# flotilla_pair()'s random_bytes(32), so it can never contain the `"` or `\` that curl's
# config-file parser treats specially inside a quoted value.
HTTP=$(printf '%s' "$BODY" | curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST \
